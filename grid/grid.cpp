    ...
    for (int i = 0; i < number_of_processes; i++) {
        ...
        for (int j = 0; j < space_dimension; j++) {
            ...
            nodes_counter *= global_node_count_per_dimension[j] / process_per_dim[j] +
                             (coordinates[j] == process_per_dim[j] - 1 ? global_node_count_per_dimension[j] %
                                                                         process_per_dim[j] : 0);
        }
...
int RegularGrid::global_multi_to_singleindex(const MultiIndex &global_multi_index) const {
    ...
    for (int i = 0; i < space_dimension; ++i) {
        ...
        local_multi_index[i] =
                global_multi_index[i] - process_coords[i] * (node_count_per_dimension_[i] / process_per_dim_[i]);
    }
    int rank;
