...
for i in range(1, 11):
    ...
    for j in range(0, solution.size()):
        ...
        index = mymodule.to_multi_index(j_2, mymodule.MultiIndex(30,30))
        point = mymodule.Point(index[0]/29,index[1]/29)
        solution[j] = x[j] - boundary_function(point ,t)
    
    gridFunc = mymodule.GridFunction(grid, solution)

    mymodule.write_to_vtk('../python/heat_timesteps_solution' + str(i), gridFunc, 'heat_timesteps_solution' )
