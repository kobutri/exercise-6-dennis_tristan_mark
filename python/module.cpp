...
PYBIND11_MODULE(pmsc, mod) {
    ...
    py::class_<SparseMatrix<scalar_t>, std::shared_ptr<SparseMatrix<scalar_t>>>(mod, "SparseMatrix")
            .def(py::init<>())
            ...
            .def(py::init(
                    [](MpiCommWrapper comm, int local_rows, int global_columns, std::function<int(int)> nz_per_row) {
    ...
    mod.def("equals",
            static_cast<bool (*)(std::shared_ptr<SparseMatrix<scalar_t>>, std::shared_ptr<SparseMatrix<scalar_t>>)>([](
                    std::shared_ptr<SparseMatrix<scalar_t>> lhs, std::shared_ptr<SparseMatrix<scalar_t>> rhs) {
    ...
    mod.def("multiply", static_cast<void (*)(Vector<scalar_t> &, std::shared_ptr<SparseMatrix<scalar_t>>,
                                             const Vector<scalar_t> &)>([](Vector<scalar_t> &res,
                                                                           std::shared_ptr<SparseMatrix<scalar_t>> lhs,
                                                                           const Vector<scalar_t> &rhs) {
                return multiply(res, *lhs, rhs);
            }), py::arg("result"),
            py::arg("lhs"), py::arg("rhs"));
    ...
    py::class_<IterativeSolver<scalar_t>, PyIterativeSolver>(mod, "IterativeSolver")
            ...
            .def("max_iterations", py::overload_cast<std::optional<int>>((void (IterativeSolver<scalar_t>::*)(
                    std::optional<int>)) &IterativeSolver<scalar_t>::max_iterations), py::arg("value"))
            ...
            .def("absolute_tolerance", py::overload_cast<>(
                    ...
                    py::const_))
            .def("absolute_tolerance", py::overload_cast<scalar_t>(
                    (void (IterativeSolver<scalar_t>::*)(scalar_t)) &IterativeSolver<scalar_t>::absolute_tolerance),
                 py::arg("value"))
    ...
    py::class_<Preconditioner<scalar_t>, PyPreconditioner, std::shared_ptr<Preconditioner<scalar_t>>>(mod,
                                                                                                      "Preconditioner")
            .def(py::init<>())
    ...
    py::class_<GaussSeidelIteration<scalar_t>, Preconditioner<scalar_t>, std::shared_ptr<GaussSeidelIteration<scalar_t>>>(
            mod, "GaussSeidelIteration")
            .def(py::init<>());

    py::class_<JacobiIteration<scalar_t>, Preconditioner<scalar_t>, std::shared_ptr<JacobiIteration<scalar_t>>>(mod,
                                                                                                                "JacobiIteration")
            .def(py::init<>());
    ...
    py::class_<RegularGrid>(mod, "RegularGrid")
            ...
            .def(py::init(
                    [](MpiCommWrapper communicator, Point min_corner, Point max_corner,
    ...
    mod.def(
            "write_to_vtk", [](const std::string &path, const GridFunction<scalar_t> &grid_function,
                               const std::string &name) { write_to_vtk(path, grid_function, name); },
    ...
    mod.def("compute_l_infinity_error", &compute_l_infinity_error<scalar_t>, py::arg("grid"),
            py::arg("computed_solution"), py::arg("analytical_solution"));

    mod.def(
            "to_global_index", [](const Vector<scalar_t> &vector, int local_index) {
                return (vector.partition()).to_global_index(local_index);
            },
